...
#define MAX_SHOT_ADDR			0x0807e000
...
// logs a shot by writing it to memory. Starting address in cur_address.
void shot_mem_write(info_shot_t shotinfo)
{
	HAL_FLASH_Unlock();
	HAL_FLASH_Program(FLASH_TYPEPROGRAM_DOUBLEWORD, cur_address, shotinfo.shot_num);
	cur_address += 8;
	HAL_FLASH_Program(FLASH_TYPEPROGRAM_DOUBLEWORD, cur_address, shotinfo.pwm1);
	cur_address += 8;
	HAL_FLASH_Program(FLASH_TYPEPROGRAM_DOUBLEWORD, cur_address, shotinfo.max_freq);
	cur_address += 8;
	HAL_FLASH_Program(FLASH_TYPEPROGRAM_DOUBLEWORD, cur_address, shotinfo.used_freq);
	cur_address += 8;
	HAL_FLASH_Program(FLASH_TYPEPROGRAM_DOUBLEWORD, cur_address, shotinfo.total_time);
	cur_address += 8;
	HAL_FLASH_Program(FLASH_TYPEPROGRAM_DOUBLEWORD, cur_address, shotinfo.batt_lvl);
	cur_address += 8;
	HAL_FLASH_Program(FLASH_TYPEPROGRAM_DOUBLEWORD, cur_address, shotinfo.max_current);
	cur_address += 8;
	HAL_FLASH_Program(FLASH_TYPEPROGRAM_DOUBLEWORD, cur_address, shotinfo.pressure);
	cur_address += 8;
	for (uint8_t i = 0; i < sizeof(shotinfo.current)/sizeof(shotinfo.current[0]); i++)
	{
		HAL_FLASH_Program(FLASH_TYPEPROGRAM_DOUBLEWORD, cur_address, shotinfo.current[i]);
		cur_address += 8;
	}
	HAL_FLASH_Lock();
}
