...
change data: 2022-12-03
added no mesh judge, AD value >= 480 stop ejection
change data: 2022-12-08
inhale  pressure sensor sampling as 10ms interval
********************************************************/
...
#define PRES_SAMPLE_VAL         5		         // pressure sensor sampling interval = 10ms
...
//180.00KHZ + 280*10HZ = 182.85KHZ
//#define DEVICE_FRC          FRC_170000HZ + 720*FRC_10HZ	    // current device tranducer frequency
...
#define FRC_170000HZ           11408496    //170.00KHZ
...
void SprayTask_Process( void )
{
    switch( s_spray_state )
    {
        ...
        case STATE_SPRAY_INHALATION:
             spray_intv = SCAN_DLY_2;
      		 delay_05s++;
      		 if(delay_05s >= 115)						  //0.21S scan 5 frequency
      		 {
      			  if(delay_05s == 115)
      			  {
      				  current_frc += FRC_10HZ*2;          //autotune(as 10HZ)
      				  ad9833_set_frq( current_frc );
      				  pwm_adc_max = 0;
      			  }
      			  else
      			  {
      	             ...
      	             if(pwm_adc_val >= CURRENT_THR1)		// added
      	             {
      	            	 ADI_OE_OFF();
      	             }
      		          if(pwm_adc_val >= pwm_adc_max)
      		          ...
      		          pwm_list_offset++;
      		          if(pwm_list_offset <= 4)						 // +20H, - 20HZ  TOTAL 50HZ
      		          {
      		               current_frc -= FRC_10HZ;					 //autotune (as 10HZ)
      		               ad9833_set_frq( current_frc );
      		          }
