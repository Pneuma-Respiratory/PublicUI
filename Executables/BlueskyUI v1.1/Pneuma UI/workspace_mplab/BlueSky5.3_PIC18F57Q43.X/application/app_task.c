...
#define SPRAY_TURN_OFF_DLY      190       // unit: 100ms; turn off after 32s = 320*100ms  150 = 30s
...
// MCLK = 4MHZ                            
#define SCAN_CNT1               50        // SCAN FREQUENCY QTY1 
...
{                                    
    ...
    {       
        ...
        case STATE_SPRAY_SCAN                                                                                                                                                                                                                                                                                                                        :                                                                        
            ...
            {
            	cnt_alarm1++;
            	if((cnt_alarm1 >= ALARM1_COUNT ) || (pwm_adc_val >= CURRENT_THR1A))
            	{
                    ADI_OE_OFF();   
                    DC8V_EN_OFF();  
                    DC24V_EN_OFF();  
        ...
        case STATE_SPRAY_INHALATION:                                
            ...
            if((current_index < CURRENT_SIZE) && (current_inc++ % current_rate == 0))
            {
       		...
       		{                                                       
                ...
                {
                    if(pwm_adc_evg > before_pwm_adc + (uln_level - before_level)*15 + UP_OFFSET)
                    {
                        cnt_alarm2++;
                    }
                }
                ...
                {
                    if(pwm_adc_evg > before_pwm_adc + DOWN_OFFSET)
                    {
                        cnt_alarm2++;
                    }
                }
	    	    ...
	    	    {         
                    cnt_adc_GT = 0;
                    if(pwm_adc_evg >= CURRENT_TARGET + 15)                                                             
                    {   
                        cnt_adc_LT = 0;
                        ...
                        curt_adc_offset2 = (pwm_adc_evg -CURRENT_TARGET)/32;
                    ...
                    {
                        if(pwm_adc_evg < CURRENT_TARGET + 10)
                        {
                            cnt_adc_LT++;
                        }
                        if((pwm_adc_evg >= CURRENT_TARGET + 10) || (cnt_adc_LT >= 2))
                        {   
                            cnt_adc_LT = 0;
                            if(duty_set_val <= PWM_DUTY_MIN)
                            {
                ...
                {   
                    if(pwm_adc_evg <= CURRENT_TARGET - 6)
                    {
                        ...
                        else
                        {
                            ...
                            if((pwm_adc_evg <= CURRENT_TARGET -10) || (cnt_adc_GT  >= 2))      
                            {                                                               
                                cnt_adc_GT = 0;                                         
                                if(duty_set_val != 0)
                                {   
...
#endif
#if CURRENT_PROTECTION_EN
                if((cnt_alarm2 >= ALARM2_COUNT)||(cnt_alarm3 >= ALARM3_COUNT)||(cnt_alarm4 >= ALARM4_COUNT))                                                                                             
                {                                                               
                    ADI_OE_OFF();
                    DC8V_EN_OFF();
                    DC24V_EN_OFF();
