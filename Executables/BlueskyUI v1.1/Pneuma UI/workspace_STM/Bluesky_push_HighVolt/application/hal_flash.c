...
// finds the total amount of shots that are logged and sets cur_address to the first free data address
...
uint16_t shot_mem_find()
{
	...
	while(data != 0xFFFFFFFF)
	{
		...
		cur_address += 4 * sizeof(info_shot_t);
	}
...
void shot_mem_write(info_shot_t shotinfo)
{
	...
	cur_address += 8;
	HAL_FLASH_Program(FLASH_TYPEPROGRAM_DOUBLEWORD, cur_address, shotinfo.freq_idx);
	cur_address += 8;
	...
	cur_address += 8;
	HAL_FLASH_Program(FLASH_TYPEPROGRAM_DOUBLEWORD, cur_address, shotinfo.current);
	cur_address += 8;
