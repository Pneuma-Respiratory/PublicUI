...
uint16_t NUMBER_OF_SHOTS = 0;       // unit: 100ms ; 5S ï¿½Ô¶ï¿½ï¿½Ø»ï¿½
uint8_t SPRAY_PWM_DUTY = 20;        // Spray percent 10*2=20ms/20ms=100%
uint8_t index1 = 0;
uint8_t sdp_thr = 0;
...
uint16_t SPRAY_EN_TIME = 5000;      // 5000*2ms = 10s
...
{   
	...
	for (i=0; i<SCAN_COLUMN; i++){
		pwm_freq_list[i] = (uint16_t) ((int) (RES_FREQ) + ind);
		ind++;
	}
    if (SPRAY_DELAY > 1) {
...
//                 if ( (p_bt_state == NULL) || (p_user_info->unused_pct == 0) ){  use if you want max amount of time within spray
#if PROG_TYPE == 3
...
#if (PROG_TYPE != 1 && PROG_TYPE != 4)
    value = HalSdp_GetThr();              //ï¿½ï¿½ï¿½ï¿½Ñ¹ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½
    if ( value > SDP_TRIGER_THR ){
        spray_triger = TRIGER_SDP;
        rst = true;
        sdp_value_max = value;            //ï¿½ï¿½Î»Ñ¹ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½Öµ
    }
#endif
#if (PROG_TYPE != 2)
...
void app_task_Init( void )
{
    ...
    SPRAY_EN_TIME = MAX_HELD;                //unit: 2ms
    SPRAY_TURN_OFF_DLY = IDLE_TIMEOUT;
    if (SPRAY_DELAY > 1)
    {
        SCAN_DLY_1 = SPRAY_DELAY;
...
void KeyTask_Process( void )
{
    ...
    if ( (sw_status == KEY_POWER_ON) && (key_value != KEY_RLS) ){
        return;
    }
    sw_status = key_value;
}
    ...
    switch( s_spray_state )
    {
        ...
        case STATE_SPRAY_READY:
             if ( sw_status == KEY_SPRESS )
             {
                  ...
                  pApp_Task->task_led.event = LED_STATE_BLINK_EVT;
                  pApp_Task->task_led.param1 = 2;
                  pApp_Task->task_led.param2 = 150;        // 150*2ms
                  pApp_Task->task_power_off.interval = 0;
                  pApp_Task->task_power_off.event = 0;
                  s_spray_state = STATE_SPRAY_ACTIVE_DLY;
                  spray_intv = SPRAY_ON_DLY;
        ...
        case STATE_SPRAY_SCAN:
             ...
             else
             {
				 ...
				 lt_en_time = sys_ticket + SPRAY_EN_TIME;
                 s_spray_state = STATE_SPRAY_ACTIVE;
             }
             break;
...
{    
    ...
    if ( lt_en_time )
    {
        if ((sw_status != KEY_SPRESS) || ( sys_ticket >= lt_en_time ))
        {
            LT_SHNDA_OFF();
            if(fg_stop_spray == 0)
            {
