...
***********************************************************************************/
/******************************************************
PCB version: BlueSky5.3_AD9833_VR REV A  20220916 (with ad9833+ crystal 4MHZ + VR(10K) )
AD9833 MCLK= crystal 4MHZ
autotune1: 3KHZ  center frequency +1KHZ, - 2KHZ ,Step frequency =100HZ
autotune2: 100hz max_frc+-50hz, Step Frequency = 10HZ

2022-09-12
Frequency scan interval have changed as : 2ms
SCAN_DLY1 = 100*2MS = 0.2S
SCAN_DLY2 = 1*2MS = 2MS ，total scan 41 Frequency
Old Program
change data: 2022-9-13
every 50ms read once ADC current value
change data: 2022-9-30
new Program
Every 250ms scan 5 times Frequency(step 10hz,+20HZ,-20HZ) , receive max as next 5 times center frequency
...
uint16_t pwm_tx_adc[120];			// uart print used
...
uint16_t adc_current_measurement(void);
    ...
    switch( s_spray_state )
    {
        ...
        case STATE_SPRAY_READY:
             if ( state_spray_ready_init() ){
                 ...
                 spray_delay_time = 0;
                 pressure_index = 0;
                 for (uint16_t i = 0; i < sizeof(shot_info.pressure)/sizeof(shot_info.pressure[0]); i++) shot_info.pressure[i] = 0;
        ...
        case STATE_SPRAY_SCAN:
             ...
             spray_intv = SCAN_DLY_2;
             pwm_adc_val = adc_current_measurement();
             adc_tx_val = swap_16bits(pwm_adc_val);

             pwm_freq_adc[pwm_adc_cnt] = pwm_adc_val;		//**** test
             pwm_tx_adc[pwm_adc_cnt] = adc_tx_val;			// txd_value
             pwm_adc_cnt++;									//**** test
             ...
             else
             {
            	 ...
            	 else
            	 {
            		 ...
            		 else
            		 {
            			 ...
            			 delay_05s = 0;									// 0.5S delay reset = 0
            			 pwm_list_offset = 0;							//
            			 sdp_cnt = 50;								    // 0.1S reset
        				 avg_current = pwm_adc_max;
        				 last_freq_used = current_frc;
            			 if ( spray_triger == TRIGER_SDP )
            			 {
#if PROG_TYPE != 6
            				 spray_intv = 25;
        ...
        case STATE_SPRAY_INHALATION:
			...
			sdp_value = 0;
             ...
             if (pressure_index < sizeof(shot_info.pressure) / sizeof(shot_info.pressure[0]))
...
void save_shot(){
 	...
 	shot_info.used_freq = last_freq_used;
    shot_info.sys_ticket = sys_ticket;
    shot_info.spray_time = spray_time;
 	for (uint8_t i = 0; i < sizeof(shot_info.autotune)/sizeof(shot_info.autotune[0]); i++){
     	shot_info.autotune[i] = pwm_freq_adc[i];
 	}
 	info_mem_write(&shot_info, sizeof(info_shot_t)/8);
...
{    
    ...
    if ( lt_en_time )
     {
    	// measure spray time and get current measurement
    	if ((spray_triger) && s_spray_state >= STATE_SPRAY_READY){
    		spray_time++;
    		if(spray_time % 50 == 0 && currentIndex < sizeof(shot_info.current)/sizeof(shot_info.current[0])){
        		adc_result = adc_current_measurement();
    			shot_info.current[currentIndex++] = adc_result;
    		}
    	    avg_current += adc_result;
            avg_current = avg_current / 2;
    	}
