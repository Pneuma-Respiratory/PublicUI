...
***********************************************************************************/
/******************************************************
PCB version: BlueSky pushmode 20220119  remark: without IDchip,with boost circuit(3.7V--> 5V~6V)
Program change: 2022-01-24
pressure sensor:reverse
...
uint16_t SCAN_DLY_1 = 1;       // 50x2ms
uint16_t SDP_STOP_PERCENT = 25;        // stop spray pressure value percent = pressure_value_max * 25%
uint16_t SPRAY_PWM_DUTY = 20;
uint16_t KEY_LP_TIME;
uint16_t SDP_TRIGER_THR;
uint16_t spray_time = 0;
...
void IntervalTask_Process(void);
...
#if PROG_TYPE == 7
    if ( pApp_Task->task_interval.event & TASK_ACTIVE_EVT ){
        pApp_Task->task_interval.event ^= TASK_ACTIVE_EVT;
        pApp_Task->task_interval.interval = IDLE_INTERVAL;
        IntervalTask_Process();
    }
#endif
...
#if PROG_TYPE == 4
    	queue_shots = lp_shots;
#elif PROG_TYPE == 7
        pApp_Task->task_interval.interval = 5;
#else
        power_off();
#endif
    }
...
void LedTask_Process( void )
{
    ...
    if ( pApp_Task->task_led.event & LED_STATE_FREQ_EVT ){
    	HalLed_Spec(pwm_mfreq_idx);
    }
    else if ( pApp_Task->task_led.event & LED_STATE_BLINK_EVT ){
...
void SprayTask_Process( void )
{
    switch( s_spray_state )
    {
        ...
        case STATE_DOSE_COMPLETE:
        	...
        	if (!shot_mem_full){
            	shot_info.shot_num++;
            	shot_info.total_time = spray_time;
            	shot_info.batt_lvl = bat_adc;
            	shot_info.freq_idx = pwm_mfreq_idx;
            	shot_info.used_freq = pwm_freq_list[pwm_mfreq_idx];
            	shot_info.current = pwm_adc_max;
            	shot_info.pressure = sdp_value_max;
            	shot_mem_write(shot_info);
        	}
        	spray_time = 0;
#if PROG_TYPE == 3
            index1 = pwm_mfreq_idx;
            pApp_Task->task_led.interval = 2;                           // ####
            pApp_Task->task_led.event = LED_STATE_FREQ_EVT;            // #### ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½Ë¸LED 2ï¿½ï¿½
#endif
             HalSdp_Stop_Measure();
             spray_triger = 0;
...
#if PROG_TYPE != 1 && PROG_TYPE != 7 && PROG_TYPE != 4 && PROG_TYPE != 9
    value = HalSdp_GetThr();
...
#endif
#if PROG_TYPE != 2 && PROG_TYPE != 7 && PROG_TYPE != 8
...
void cart_Detect( void )
{
	...
	p_user_info->per_key_time = p_cart_attr->per_key_time = SPRAY_ON_TIME;
...
{    
    ...
    if ( lt_en_time )
    {
    	...
    	{
    		if(fg_stop_spray == 0)                      //ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½
    	    {
...
#if PROG_TYPE == 7
	if ( pApp_Task->task_interval.interval > 0 && s_spray_state == STATE_SPRAY_READY)
	{
		if (sys_ticket % 500 == 0)
		{
			if ( --pApp_Task->task_interval.interval == 0 )
			{
				pApp_Task->task_interval.event |=  TASK_ACTIVE_EVT;
				eventFlag = true;
			}
